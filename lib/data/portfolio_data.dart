...
class PortfolioData {
  ...
  static const List<TechModel> techStack = [
    TechModel(icon: '🐦', name: 'Flutter / Dart', pct: 0.95),
    TechModel(icon: '🔥', name: 'Firebase',       pct: 0.88),
    TechModel(icon: '⚙️', name: 'GetX',           pct: 0.90),
    TechModel(icon: '🔗', name: 'REST API',        pct: 0.85),
    TechModel(icon: '📐', name: 'Clean Arch.',     pct: 0.82),
  ];
  ...
  static final List<ProjectModel> projects = [
    ProjectModel(
      title: 'Smart Job Circular App', emoji: '💼', featured: true,
      ...
      tagColors: ['cyan','purple','purple','green','cyan'],
      githubUrl: null, isPrivate: true,
    ),
    ProjectModel(
      title: 'E-Learning Platform', emoji: '🎓', featured: false,
      ...
      tagColors: ['cyan','purple','green'],
      githubUrl: 'https://github.com/Ashikur-Arman/E-Learnig-App', isPrivate: false,
    ),
    ProjectModel(
      title: 'Wholesale Platform', emoji: '📊', featured: false,
    ...
    // ProjectModel(
    //   title: 'My New App', emoji: '🚀', featured: false,
    //   desc: 'Description here.',
    //   impact: 'Impact here.',
    //   tags: ['Flutter','Firebase'],
    //   tagColors: ['cyan','purple'],
  ...
  static final List<SkillCatModel> skills = [
    SkillCatModel(cat: 'Mobile Development', color: 'cyan',
        skills: ['Flutter','Dart','Android','iOS']),
    SkillCatModel(cat: 'State Management', color: 'purple',
        skills: ['GetX','GetStorage']),
    SkillCatModel(cat: 'Backend / Cloud', color: 'green',
        skills: ['Firebase Auth','Firestore','Storage']),
    SkillCatModel(cat: 'API Integration', color: 'amber',
        skills: ['REST API','Dio','JSON Parsing']),
    SkillCatModel(cat: 'Software Engineering', color: 'pink',
        skills: ['Clean Architecture','Responsive UI','Performance Opt.']),
    SkillCatModel(cat: 'Tools', color: 'cyan',
        skills: ['Git','GitHub','VS Code','Postman']),
  ];

  // ── ACHIEVEMENTS — add new AchievementModel blocks here ───────
  static final List<AchModel> achievements = [
    AchModel(icon: '🏆', title: '2nd Runner-up',
        sub: 'Rangpur Divisional Project Competition (HSTU)'),
    AchModel(icon: '💻', title: 'Codeforces Rating: 883',
        sub: 'Solved 150+ competitive programming problems'),
    AchModel(icon: '🌐', title: 'Open-Source Contributor',
        sub: 'Active contributor on GitHub'),
    AchModel(icon: '🎓', title: 'Active CSE Club Member',
        sub: 'HSTU — Dinajpur'),
  ];
}
...
class TechModel {
  ...
  final double pct;
  const TechModel({required this.icon, required this.name, required this.pct});
}
...
class ProjectModel {
  final String title, emoji, desc, impact;
  final List<String> tags, tagColors;
  final String? githubUrl;
  final bool isPrivate, featured;
  const ProjectModel({
    required this.title, required this.emoji, required this.desc,
    required this.impact, required this.tags, required this.tagColors,
    required this.githubUrl, required this.isPrivate, required this.featured,
  });
}

class SkillCatModel {
  final String cat, color;
  final List<String> skills;
  const SkillCatModel({required this.cat, required this.color, required this.skills});
}

class AchModel {
  final String icon, title, sub;
  const AchModel({required this.icon, required this.title, required this.sub});
}
